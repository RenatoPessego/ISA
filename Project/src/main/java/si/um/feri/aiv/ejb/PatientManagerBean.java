...
public class PatientManagerBean implements PatientManager {
    ...
    @Override
// This method is called by the MDB to assign a doctor if they still have room
    public boolean assignDoctorIfPossible(int patientId, int doctorId) {
        try {
            ...
            // Count how many patients are currently assigned to this doctor
            int currentCount = em.createQuery(
                            "SELECT COUNT(p) FROM Patient p WHERE p.doctor = :doctor", Long.class)
                    .setParameter("doctor", doctor)
                    .getSingleResult()
                    .intValue(); // Convert from Long to int

            // Print for debugging
            System.out.println("Doctor " + doctorId + " has " + currentCount + " / " + doctor.getMaxPatients() + " patients.");

            // If the doctor is full, return false
            if (currentCount >= doctor.getMaxPatients()) {
                System.out.println("Doctor has reached the maximum number of patients.");
                return false;
            }
