...
import si.um.feri.aiv.jms.DoctorResponseRegistry;
...
//    }
    ...
    @EJB
    private DoctorSelectionSender doctorSelectionSender;

    // This method is called from the JSF button to send the JMS request
    @Override
    public String sendDoctorSelectionRequest() {
        if (selectedPatient != null && selectedPatient.getDoctor() != null) {
            int patientId = selectedPatient.getId();
            int doctorId = selectedPatient.getDoctor().getId();

            doctorSelectionSender.sendDoctorSelectionRequest(patientId, doctorId);
            System.out.println("Doctor selection request sent from JSF page.");

            try {
                // Wait briefly for the MDB to process and respond (simulado)
                Thread.sleep(1000);

                String response = DoctorResponseRegistry.getMessage(patientId);
                ...
                if (response != null) {
                    FacesContext.getCurrentInstance().addMessage(null,
                            new FacesMessage(FacesMessage.SEVERITY_INFO, response, null));
                    DoctorResponseRegistry.removeMessage(patientId); // cleanup
                } else {
                    FacesContext.getCurrentInstance().addMessage(null,
                            new FacesMessage(FacesMessage.SEVERITY_WARN, "Awaiting response from doctor...", null));
                }
            ...
            FacesContext.getCurrentInstance().getExternalContext().getFlash().setKeepMessages(true);
            FacesContext.getCurrentInstance().addMessage(null,
                    new FacesMessage(FacesMessage.SEVERITY_ERROR, "Please select a doctor before submitting.", null));
        }
